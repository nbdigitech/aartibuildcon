            ...
            <div class="col-md-12">
              <div class="card">
                  ...
                  <div style="background-color: #274CBF; padding: 10px; padding-left: 16px; padding-top:16px; border-radius: 3px;" style="width: 100%;">
                    <h4>Manage Other Leads Users
                    <a href="{{route('Admin/FacebookLead/Add')}}" class="btn btn-info pull-right" style="margin-top:-7px; background:white; color:black;">Add New</a>
                      </h4>
                  </div>
                </div>
                <div class="card-body">
              ...
              @endif
                    ...
                    <table id="datatables" class="table table-no-bordered" cellspacing="0" width="100%" style="width:100%">
                      <thead>
                        <tr>
                          ...
                          <th style="font-weight: bold;">Interested</th>
                          <th style="font-weight: bold;">SiteVisit</th>
                          <th style="font-weight: bold;">Source</th>
                          <th style="font-weight: bold;">SendSMS</th>
                          <th style="font-weight: bold;">SendEmail</th>
                  ...
                  @foreach($applyuser as $row)
                        <tr>
                          ...
                          <td>{{$row->Source}}</td>
                           <form action="{{route('Admin/FacebookLead/EditSMSSession')}}" method="post">
                              {{csrf_field()}}
                            <td class="td-actions">
                          ...
                          <form action="{{route('Admin/FacebookLead/EditMailSession')}}" method="post">
                              {{csrf_field()}}
                            <td class="td-actions">
                              <input type="hidden" name="Edit" value="{{$row->id}}">
                            <button type="submit" rel="tooltip" class="btn btn-success" style="background-color: #274CBF; padding:10px; padding-left: 20px; box-shadow: none; margin-right: 10px; padding-right: 20px;">
                            ...
                            <form action="{{route('Admin/FacebookLead/EditSession')}}" method="post">
                              {{csrf_field()}}
                            <td class="td-actions">
                              <input type="hidden" name="Edit" value="{{$row->id}}">
                            <button type="submit" rel="tooltip" class="btn btn-success" style="background-color: #274CBF; padding:10px; padding-left: 20px; box-shadow: none; margin-right: 10px; padding-right: 20px;">
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="row" style="padding-bottom: 50px;">
        <div class="col-sm-2"></div>
        <div class="col-sm-4">
          <form action="{{route('Admin/FacebookLead/Delete')}}" method="post">
            {{csrf_field()}}
